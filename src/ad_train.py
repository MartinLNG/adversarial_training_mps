...
def _adversarial_training_step(dis,  # TODO: define abstract discriminator class
    ...
    synth_X = sampling.batch(mps=mps,
    ...
    if check:
        # save only one tensor for later
        l_tensor_comp = mps.tensors[0][0, 0].detach().numpy()
...
def ad_train_loop(dis: nn.Module,  # TODO: define abstract discriminator class,
                  ...
                  # one embedding per class. Just use basis embedding, right?
                  cls_embs: Sequence[torch.Tensor],

                  acc_drop_tol: float,
                  recompute_best_acc: bool,  # allowed drop in accuracy, might be zero
                  # number of accuracy checks per epoch (going through the original dataste)
                  num_acc_check: int,
    ...
    mps.to(device)
    best_acc = mps_cat.acc_eval(mps, mps_cat_loader, 'valid', device)
    ...
    for epoch in range(ad_max_epoch):
        ...
        if check_counter == num_acc_check:
            ...
            acc = mps_cat.acc_eval(mps, mps_cat_loader, 'valid', device)
            cat_acc.append(acc)
            ...
            if acc < (best_acc - acc_drop_tol):
                ...
                (best_tensors, _,
                 val_accuracy) = mps_cat.train(mps=mps,
