...
# ------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------
# --------------------DISCRIMINIATOR ARCHITECTURES-----------------------------------------------
# ------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------

class DiscriminatorFunction(nn.Module):
    def __init__(self, cfg: schemas.CriticConfig, data_dim: int, num_cls: int):
        self.data_dim, self.num_classes = data_dim, num_cls
        self.architecture_name = cfg.architecture


class MLP(DiscriminatorFunction):
    def __init__(self, cfg: schemas.CriticConfig, data_dim: int, num_cls: int):
        super().__init__(cfg, data_dim, num_cls)
...
# ADDED AS ISSUE.
# TODO: Add other discriminator functions, e.g. MPS with MLP module
...
def init_architecture(
        cfg: schemas.CriticConfig,
        data_dim: int,
        num_classes: int
        ) -> Dict[Any, DiscriminatorFunction]:
    
    architecture_key = cfg.architecture.lower().replace("-", "").replace(" ", "")
    architecture_cls = _ARCHITECTURE_MAPPING.get(architecture_key, None)
    if architecture_key is None:
        raise KeyError(f"{cfg.architecture} not recognised.") 
    if cfg.mode == "single":
        return {"single": architecture_cls(cfg, data_dim)}
    elif cfg.mode == "ensemble":
        assert num_classes > 0, f"{num_classes=}"
        return {str(c): architecture_cls(cfg, data_dim) for c in range(num_classes)}
    else:
        raise KeyError(f"{cfg.mode} not recognised.")

# TODO: Write a function, that on a very basic level takes a dictionary of nn.Module sublasses and has a forward depending 
#       on the mode of the discriminator (ensemble vs single)
# TODO: Discriminator includes the loss definition (Crossentropy, WGAN),
#       this is in the end what distinguishes different gan trainings from each other.
...
class Critic:
    def __init__(self, cfg: schemas.GANStyleConfig, datahandler: DataHandler):
        # Storing some information for convenience
        self.model_cfg = cfg.critic
        self.train_cfg = cfg.discrimination
        self.mode = cfg.critic.mode
        self.data_dim = datahandler.data_dim
        self.num_classes = datahandler.num_cls

        # This initalized the type of critic used.
        self.archs = nn.ModuleDict(init_architecture(cfg, self.data_dim, self.num_classes))
        self.criterion = get.criterion(self.model_cfg.criterion)

        # Initialize optimizer here, as critic train will be called multiple times.
        self.optimizer = get.optimizer(self.archs.parameters(), self.train_cfg.optimizer) # think about initializing this once

    def forward(self, data: torch.FloatTensor):
        # assume that data has shape: (batch_size, num_classes, data_dim)
        batch_size, num_classes, data_dim = data.shape
        output = {}
        if self.mode == "single":
            data = data.view(batch_size, num_classes*data_dim)
            output["single"] = self.archs["single"].forward(data)
            return output
        else:
            for c in self.archs.keys():
                output[c] = self.archs[c].forward(data)
            return output

    def critic_bce_loss(self, 
                    natural: torch.FloatTensor, 
                    generated: torch.FloatTensor): # to optimize itself, maybe only needed in the train method
        return NotImplementedError
    
    def critic_wgan_gp(self, 
                       natural: torch.FloatTensor, 
                       generated: torch.FloatTensor): # to optimize generator, needed outside this class for the gan style trainer
        
        return NotImplementedError
    
    def _gradient_penality(self, 
                            natural: torch.FloatTensor, 
                            generated: torch.FloatTensor): # for WGAN-GP critic
        return NotImplementedError
    
    def generator_loss()
    
    def train(self, stage: str,
              natural: torch.FloatTensor, 
              generated: torch.FloatTensor): # Only inner loop of GAN-style training, meaning the optimization of the discriminator
        
        if stage=="pre":
            max_epoch = self.train_cfg.max_epoch_pre
        elif stage=="gan":
            max_epoch = self.train_cfg.max_epoch_gan
        else:
            raise KeyError(f"'stage' can be either 'pre' or 'gan', not '{stage}'.")
        
        # TODO: What is the shape, object, with which the data is handled? class identity of data has to be known. 
        #       thus either use label or well chosen tensor shape
        return NotImplementedError
