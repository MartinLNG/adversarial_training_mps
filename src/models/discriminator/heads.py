...
class AwareHead(nn.Module):
    def __init__(self, feature_dim: int, num_cls: int):
        super().__init__()
...
class AwareLinearHead(AwareHead):
    def __init__(self, feature_dim: int, num_cls: int):
        super().__init__(feature_dim, num_cls)
...
def get_head(class_aware: bool, name: str, model_kwargs: dict, 
             feature_dim: int, num_cls: int) -> AwareHead | AgnosticHead:
    name=name.lower().replace("-", "").replace(" ", "")
    if class_aware:
        return _AWARE_HEADS[name](feature_dim, num_cls, **model_kwargs)
    else:
        return _AGNOSTIC_HEADS[name](feature_dim, **model_kwargs)