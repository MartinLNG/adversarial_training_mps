...
def main(cfg: schemas.Config):
    ...
    for n in generator.mats_env:
        if n.is_leaf:
            logger.info(f"\nnode {n.name} is leaf\n") 
        else: 
            logger.error(f"\nnode {n.name} is not leaf\n")
    ...
    logger.info("\nData for pretraining of discriminator loaded.\n")
