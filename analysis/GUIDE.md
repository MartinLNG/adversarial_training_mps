...
python analysis/queue_analysis.py --filter-experiment alpha_curve
...
# 5. Analyze a specific alpha_curve sweep (no viz, as queue_analysis does)
python analysis/sweep_analysis.py outputs/alpha_curve/gen/legendre/d10D6/circles_4k_1404 --no-viz
```
...
Discovers every directory under `outputs/seed_sweep/` **and** `outputs/alpha_curve/` that contains `multirun.yaml`, checks whether `analysis/outputs/<sweep>/evaluation_data.csv` already exists, and runs `sweep_analysis.py` for each unanalyzed sweep.

**Distribution plots are never generated by `queue_analysis.py`** — `--no-viz` is always passed to `sweep_analysis.py`. Use `queue_visualize.py` to regenerate plots separately.
...
python analysis/queue_analysis.py --filter-experiment alpha_curve
python analysis/queue_analysis.py --filter-experiment seed_sweep
...
python analysis/queue_analysis.py --filter-embedding legendre
...
# Combine filters (AND)
python analysis/queue_analysis.py --filter-experiment alpha_curve --filter-embedding legendre

# Force re-run even if evaluation_data.csv already exists
python analysis/queue_analysis.py --force
python analysis/queue_analysis.py --force --filter-experiment alpha_curve
```
...
Pass `--no-viz` on the command line to skip distribution plots without editing the file (this is what `queue_analysis.py` always does — use `queue_visualize.py` to generate plots separately).
...
> **Note:** `queue_analysis.py` always passes `--no-viz`, so distribution plots are never written during batch analysis. Run `queue_visualize.py` afterwards to generate them.
...
analysis/outputs/{seed_sweep|alpha_curve}/{type}/{emb}/{arch}/{dataset}_{date}/
  ...
  ├── best_class_dist.png          ← only if COMPUTE_DISTRIBUTIONS (not via queue_analysis)
  └── best_joint.png               ← only if COMPUTE_DISTRIBUTIONS (not via queue_analysis)
```
